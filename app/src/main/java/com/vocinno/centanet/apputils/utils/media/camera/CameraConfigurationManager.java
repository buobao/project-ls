...
package com.vocinno.centanet.apputils.utils.media.camera;
...
public final class CameraConfigurationManager {
	...
	private static Point findBestPreviewSizeValue(
		...
		for (String previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
			...
			boolean isScaleRight = ((double) newX / (double) screenResolution.y == (double) newY
					/ (double) screenResolution.x);
			Log.d(TAG, isScaleRight + "##" + newX + " / " + screenResolution.y
					+ " == " + newY + " / " + screenResolution.x);
		...
		/*
		 * CharSequence previewSizeValueString, Point screenResolution) { int
		 * bestX = 0; int bestY = 0; int diff = Integer.MAX_VALUE; for (String
		 * previewSize : COMMA_PATTERN.split(previewSizeValueString)) {
		 * 
	...
	private void setZoom(Camera.Parameters parameters) {
		...
		if (maxZoomString != null) {
			try {
				int tenMaxZoom = (int) (20.0 * Double// //
		...
		if (motZoomStepString != null) {
			try {
				double motZoomStep = Double.parseDouble(motZoomStepString
						.trim());
				int tenZoomStep = (int) (20.0 * motZoomStep);// //
				if (tenZoomStep > 1) {
		...
		if (maxZoomString != null || motZoomValuesString != null) {
			parameters.set("zoom", String.valueOf(tenDesiredZoom / 20.0));// //
		}
