...
public class NewAlbumHelper {
	...
	HashMap<String, NewImageBucket> bucketList = new HashMap<String, NewImageBucket>();

	private static NewAlbumHelper instance;

	private NewAlbumHelper() {
	}

	public static NewAlbumHelper getHelper() {
		if (instance == null) {
			instance = new NewAlbumHelper();
		}
		return instance;
	}
	 ...
	 */
	void getAlbum() {
		String[] projection = { BaseColumns._ID, AlbumColumns.ALBUM, AlbumColumns.ALBUM_ART, AlbumColumns.ALBUM_KEY, AlbumColumns.ARTIST,
	...
	void buildImagesBucketList() {
		...
		// 构造相册索引
		String columns[] = new String[] { BaseColumns._ID, ImageColumns.BUCKET_ID, ImageColumns.PICASA_ID, MediaColumns.DATA, MediaColumns.DISPLAY_NAME,
		...
		if (cur.moveToFirst()) {
			...
			do {
				...
				NewImageBucket bucket = bucketList.get(bucketId);
				if (bucket == null) {
					bucket = new NewImageBucket();
					bucketList.put(bucketId, bucket);
					bucket.imageList = new ArrayList<NewImageItem>();
					bucket.bucketName = bucketName;
				}
				bucket.count++;
				NewImageItem imageItem = new NewImageItem();
				imageItem.imageId = _id;
				imageItem.imagePath = path;
				imageItem.thumbnailPath = thumbnailList.get(_id);
				imageItem.lastModified = new File(path).lastModified();
		...
		Iterator<Entry<String, NewImageBucket>> itr = bucketList.entrySet().iterator();
		while (itr.hasNext()) {
			Map.Entry<String, NewImageBucket> entry = itr.next();
			NewImageBucket bucket = entry.getValue();
			Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", " + bucket.count + " ---------- ");
			for (int i = 0; i < bucket.imageList.size(); ++i) {
				NewImageItem image = bucket.imageList.get(i);
	 ...
	 * @param refresh
	 */
	public List<NewImageBucket> getImagesBucketList(boolean refresh) {
//		if (refresh || (!refresh && !hasBuildImagesBucketList)) {
		bucketList.clear();
			buildImagesBucketList();
//		}
		List<NewImageBucket> tmpList = new ArrayList<NewImageBucket>();
	...
	String getOriginalImagePath(String image_id) {
		...
		Cursor cursor = cr.query(Media.EXTERNAL_CONTENT_URI, projection, BaseColumns._ID + "=" + image_id, null, null);
		if (cursor != null) {
			cursor.moveToFirst();
