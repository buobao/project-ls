...
public class LeftFragment extends BaseSlidingFragment {
	...
	public final static Class[] FRAGMENTS_CLASSES = { MainHallFragment.class,
			FollowFragment.class, RankFragment.class };
	...
	public void initViews() {
		...
		mMenuLayouts = new View[] { findViewById(R.id.menu_livehall_layout),
				findViewById(R.id.menu_follow_layout),
				findViewById(R.id.menu_rank_layout) };

		mMenuIcons = new ImageView[] {
				(ImageView) findViewById(R.id.menu_livehall_icon),
				(ImageView) findViewById(R.id.menu_follow_icon),
				(ImageView) findViewById(R.id.menu_rank_icon) };

		mMenuTexts = new TextView[] {
				(TextView) findViewById(R.id.menu_livehall_text),
				(TextView) findViewById(R.id.menu_follow_text),
				(TextView) findViewById(R.id.menu_rank_text), };
	...
	private void clearMenu() {
		for (int i = 1; i <= mMenuLayouts.length; i++) {
			...
			mMenuTexts[i - 1].setTextColor(getResources().getColor(
					R.color.gray7));
		}
	}
	...
	private void setMenuChecked(int index) {
		...
		if (index != 1 && index != 2) {
			mMenuLayouts[index]
					.setBackgroundResource(R.drawable.sliding_menu_checked_bg);
		}
