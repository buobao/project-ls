...
public class ImageCompress {
    ...
    public Bitmap compressFromUri(Context context,
                                  CompressOptions compressOptions,String filePath) {
...
//        String filePath = getFilePath(context, compressOptions.uri);
    ...
    private void compressFile(CompressOptions compressOptions, Bitmap bitmap) {
       ...
       /* OutputStream stream = null;
        ...
        bitmap.compress(compressOptions.imgFormat, 50, baos);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
        int options = 50;
...
//        Bitmap bitmap2 = BitmapFactory.decodeStream(isBm, null, null);//把ByteArrayInputStream数据生成图片
        bitmap.compress(compressOptions.imgFormat,options,stream);
        Log.i("size","=========size");
    }
