...
import com.vocinno.centanet.keymanage.adapter.KeyListAdapter2;
...
import com.vocinno.centanet.tools.SwipeMenu.SwipeMenu;
import com.vocinno.centanet.tools.SwipeMenu.SwipeMenuCreator;
import com.vocinno.centanet.tools.SwipeMenu.SwipeMenuItem;
import com.vocinno.centanet.tools.SwipeMenu.SwipeMenuListView;
...
public class KeyManageActivity2 extends OtherBaseActivity implements
		IXListViewListener {
	private View mBack, mAddBorrowKey;
	private SwipeMenuListView smListView;
	private KeyListAdapter2 mListAdapter;
	...
	private void getData(int pageNo,boolean isFirst, final boolean isRefresh) {
		if(isFirst){
			Loading.showForExit(this, true);
		}
		URL= NetWorkConstant.PORT_URL+ NetWorkMethod.mtnList;
		...
		OkHttpClientManager.getAsyn(URL, map, new OkHttpClientManager.ResultCallback<String>() {
			...
			public void onError(Request request, Exception e) {
				smListView.stopRefresh();
				smListView.stopLoadMore();
			...
			public void onResponse(String response) {
				...
				if (jsReturn.isSuccess()) {
					...
					if (mKeyListData!=null&&mKeyListData.size() < MyConstant.pageSize) {
						smListView.setPullLoadEnable(false);
					}
					if (isRefresh) {
						mListAdapter.setListDatas(mKeyListData);
					} else {
						mListAdapter.addListDatas(mKeyListData);
						page++;
					}
				} else {
	...
	public int setContentLayoutId() {
		return R.layout.activity_key_manage2;
	}
	...
	public void initView() {
		...
		smListView = (SwipeMenuListView) findViewById(R.id.smlv_key_listview);
//		smListView.setCloseInterpolator(new BounceInterpolator());
		smListView.setOpenInterpolator(new BounceInterpolator());
		setSwipeMenuCreator();
		setListener();
	}
	...
	private void setSwipeMenuCreator() {
		SwipeMenuCreator creator = new SwipeMenuCreator() {
			...
			public void create(SwipeMenu menu) {
				...
				int viewType=menu.getViewType();
					switch (menu.getViewType()) {
						case 0://借用
							...
							openItem.setWidth(MyUtils.px2dip(KeyManageActivity2.this, 350));
							openItem.setIcon(R.drawable.img_guihuan);
						...
						case 1://在店
							SwipeMenuItem openItem1 = new SwipeMenuItem(
									getApplicationContext());
							openItem1.setBackground(new ColorDrawable(Color.rgb(0xC9, 0xC9, 0xCE)));
							openItem1.setWidth(MyUtils.px2dip(KeyManageActivity2.this, 350));
							openItem1.setIcon(R.drawable.img_tuihuan);
				...
				if(viewType==0){
					...
					deleteItem.setWidth(MyUtils.px2dip(KeyManageActivity2.this, 350));
					deleteItem.setIcon(R.drawable.img_pin);
		...
		smListView.setOnMenuItemClickListener(new SwipeMenuListView.OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(int position, SwipeMenu menu, int index) {
					switch (index) {
						case 0://归还
							if(menu.getViewType()==0){//借用
								MyToast.showToast(0+"-归还-"+position);
							}else if(menu.getViewType()==1){//在店
								MyToast.showToast(1+"-退还--"+position);
							}
							break;
						case 1://pin
							MyToast.showToast(1+"=pin="+position);
							intent.setClass(KeyManageActivity2.this, KeyPinDetailActivity.class);
							intent.putExtra(MyConstant.keyObj,mListAdapter.getItem(position));
	...
	public void setListener() {
		...
		smListView.setXListViewListener(this);

		smListView.setOnTouchListener(new OnTouchListener() {
		...
		smListView.setOnTouchListener(null);

	}

	@Override
	public void initData() {
		mListAdapter = new KeyListAdapter2(KeyManageActivity2.this);
		smListView.setAdapter(mListAdapter);
		smListView.setPullLoadEnable(false);
		// 添加通知
		getInitData();
	}
