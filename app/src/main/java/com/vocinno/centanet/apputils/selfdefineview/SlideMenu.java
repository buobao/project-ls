...
public class SlideMenu extends ViewGroup {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (xMoved || yMoved) {
				// 在侧边 可以滑动 并且此时在地图上
				if (mLastMotionX < 60 || mCurrentScreen == 0) {
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) {
			...
			clearChildrenCache();
			// 下句可不用 与自定义控件无关
			// MainOrderActivity.changeSlideMenuBg(mCurrentScreen == 0);
		}
	}
	...
	protected void snapToScreen(int whichScreen) {
		...
		mScroller.startScroll(getScrollX(), 0, delta, 0, Math.abs(delta) * 1);// 最后一个参数是动画时间
		invalidate();
	}
